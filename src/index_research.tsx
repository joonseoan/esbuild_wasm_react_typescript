...
const App: React.FC = () => {
  ...
  */
  ...
  // able to attributes even from iframe
  ...
  const onClick = async () => {
    ...
    // we can get html variable here because the function is an object even though html is initiated below.
    ...
    // srdoc: is same as the one in <iframe srcDoc={html} ref={iframe} /> ==> document.querySelector(iframe).srdoc
    ...
    //     that eventully makes the app have a single entry file enclosing "other files" and "3rd party lib". ]
    ...
    /*
      ...
      (for any reason, for instance, no same-origin in sandbox),
      the parrent document and the child document are still able to be interact 
        and set/get the value each other through addEventListener(set/on) and postMessage(get/emit).
      ...
      OUR APP HAS DIFFERENT, NO SAME ORIGIN BUT STILL HAS SAME PORT, SAME PROTOCOL, AND DOMAIN.
      ...
      // for child to send message to parent addEventListener 
      parent.postMessage('Hello there', '*');

      // for child to access to parent value
      parent.a

      // for parent to send messate to child addEventLietener
      [set child document]
...
*/
    ...
    // iframe: set child document
    // constentVindow: basic attribute that the parent document is able to send message to child document
  ...
  return (
    <div>
      ...
      {/* 
        ...
        it is deleted since iframe can console the codes
      */}
      {/* <pre>{code}</pre> */}
      {/* 
        [iframe]
        ...
        In other words, the parent document should not be affected by the codes user enters and simultaneously
        the code error from user must show up in the iframe (without any hang-up)
